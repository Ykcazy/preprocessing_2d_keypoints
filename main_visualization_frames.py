...
# =================================================================================
# --- REMOVED 3D VISUALIZATION FUNCTION ---
# =================================================================================

# =================================================================================
# MAIN VISUALIZATION FUNCTION (2D ONLY)
# =================================================================================
def run_visualization():
    print("🚀 Starting 2D visualization with every-frame detection and tracker-stabilized boxes.")

    # --- MODIFIED: Define and create output directories for 2D only ---
    OUTPUT_DIR_2D = "output/2d_frames"
    os.makedirs(OUTPUT_DIR_2D, exist_ok=True)
    print(f"🖼️  Saving 2D frames to: {OUTPUT_DIR_2D}")
    ...
    # 1. --- INITIALIZATION (MODIFIED FOR 2D ONLY) ---
    # We ignore the 3rd return value (pose3d_model)
    ...
    video_writer = None
    frame_idx = 0
    # This will hold the tracker results from the previous frame
    ...
    while True:
        ...
        # --- REMOVED 3D POSE LIFTING BLOCK ---

        # --- FRAME DRAWING (2D ONLY) ---
        processed_frames = []
        for i, frame in enumerate(frames):
        ...
        if display_img.size > 0:
            frame_2d_filename = os.path.join(OUTPUT_DIR_2D, f'frame_{frame_idx:05d}.png')
            cv2.imwrite(frame_2d_filename, display_img)

        # --- VIDEO WRITING AND DISPLAY ---
    ...
    print(f"✅ Processing finished. Video saved to {Config.OUTPUT_VIDEO_PATH}")
    print(f"✅ 2D frames saved to the '{OUTPUT_DIR_2D}' directory.")
